...
// global variables
float degrees[] = {360,355.5,351.0,346.5,342.0,337.5,333.0,328.5,324.0,319.5,315.0,310.5,306.0,301.5,
...
float xavgs[] = {0.198,0.162,0.134,0.067,0.031,0.005,-0.048,-0.046,-0.063,-0.148,-0.229,-0.231,-0.266,
...
float yavgs[] = {-0.342,-0.388,-0.425,-0.458,-0.499,-0.501,-0.536,-0.513,-0.607,-0.57,-0.575,-0.572,
...
// made this with help from https://playground.arduino.cc/Main/MultiMap/
float multiMap(float * val, float * degrees, float * xavgs, float * yavgs){
  /**
   * PARAMETERS
   *  val: an [x,y] pair
   *  degrees: the array of possible degrees (descending)
   *    360 is north, 270 is east, 180 is south, 90 is west
   *  xavgs: the array of xaccel values for a given direction
   *  yavgs: the array of yaccel values for a given direction
   */
  if(val[0] < -1.1 || val[0] > 1.1) return 0.0;
  if(val[1] < -1.1 || val[1] > 1.1) return 0.0;
